...
//        int duration = 3;
            ...
            list.add(new Thread(()->{
                while (true) {
                    try {
                        ...
                        String s = "http://"+paltformHost+":20000/api/v1/stream/start?serial="+paltformId+"&code="+head + inta;

                        String i1 = doPostRquest(s, token);
                        log.debug("{} --->{}",i1,s);
                        Thread.sleep(TimeUnit.SECONDS.toMillis(duration));
                        s = "http://"+paltformHost+":20000/api/v1/stream/stop?serial="+paltformId+"&code="+head + inta;
                        doPostRquest(s,token);
                    } catch (Exception e) {
    ...
    //HttpClient 超时配置
   private static String doPostRquest(String uri, String token) {
        //创建一个GET请求
