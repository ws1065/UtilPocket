...
public class Code03_PrintAllPermutations {
    ...
    public static ArrayList<String> start(String str){
        ArrayList<String> list = new ArrayList<>();
        if (str != null || str.length()>0){
            char[] chars = str.toCharArray();

            processw(chars,0,list);
        }
        return list;

    }

    private static void processw(char[] chars, int index, ArrayList<String> res) {
    ...
    private static void processf(char[] chars, int index, ArrayList<String> res) {
        if (index == chars.length){
            res.add(String.valueOf(chars));
        }
